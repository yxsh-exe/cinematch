...
import { UserIcon, EnvelopeSimpleIcon, LockIcon, UserPlusIcon, FilmSlateIcon, EyeIcon, EyeSlashIcon } from "@phosphor-icons/react";
    ...
    <div className="flex flex-col items-center justify-center min-h-[90vh] bg-background px-4 page-enter">
      <div className="w-full max-w-md p-8 sm:p-10 md:p-12 bg-card border border-border rounded-xl shadow-lg animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            ...
            <p className="font-serif text-muted-foreground text-sm mt-1">
              Join CineMatch and discover great movies
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          <div className="flex flex-col gap-1.5">
            ...
            <div className="relative">
              <UserIcon size={16} weight="duotone" className="absolute left-3.5 top-1/2 -translate-y-1/2 text-muted-foreground" />
              <input
                id="reg-username"
                type="text"
          ...
          <div className="flex flex-col gap-1.5">
            ...
            {password.length > 0 && password.length < 6 && (
              <p className="text-xs text-destructive mt-0.5">Password must be at least 6 characters</p>
            )}
          </div>
        ...
        <div className="mt-8 text-center text-sm text-muted-foreground">
          ...
          <Link to="/login" className="text-foreground font-semibold underline underline-offset-4 hover:opacity-70 transition-opacity">
            Sign in
          </Link>
        </div>
      </div>
